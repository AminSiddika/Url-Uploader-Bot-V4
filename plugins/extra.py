...
def direct_link_generator(link: str):
    ...
    if 'youtube.com' in domain or 'youtu.be' in domain:
        ...
        raise Exception('ERROR: R.I.P Zippyshare')
    else:
        raise Exception(
...
def uptobox(url: str) -> str:
    ...
    try:
        ...
        raise Exception("No Uptobox links found")
    ...
    if res['statusCode'] == 0:
        ...
        raise Exception(f"ERROR: {res['message']}")
    try:
...
def mediafire(url: str) -> str:
    ...
    if not (final_link := findall(r"\'(https?:\/\/download\d+\.mediafire\.com\/\S+\/\S+\/\S+)\'", page)):
        raise Exception("ERROR: No links found in this page")
    return final_link[0]
...
def osdn(url: str) -> str:
    ...
    try:
        ...
        raise Exception("No OSDN links found")
    cget = create_scraper().request
    try:
        page = BeautifulSoup(
...
def github(url: str) -> str:
    ...
    try:
        ...
        raise Exception("No GitHub Releases links found")
    cget = create_scraper().request
    download = cget('get', url, stream=True, allow_redirects=False)
    try:
        return download.headers["location"]
    except KeyError:
        raise Exception("ERROR: Can't extract the link")
...
def letsupload(url: str) -> str:
    ...
    if direct_link := findall(r"(https?://letsupload\.io\/.+?)\'", res.text):
        ...
        raise Exception('ERROR: Direct Link not found')
...
def onedrive(link: str) -> str:
    ...
    try:
        ...
        raise Exception(f"ERROR: {e.__class__.__name__}")
    if not link_data:
        raise Exception("ERROR: Unable to find link_data")
    folder_id = link_data.get('resid')
    if not folder_id:
        raise Exception('ERROR: folder id not found')
    folder_id = folder_id[0]
    authkey = link_data.get('authkey')
    if not authkey:
        raise Exception('ERROR: authkey not found')
    authkey = authkey[0]
    boundary = uuid4()
...
def terabox(url) -> str:
    if not path.isfile('terabox.txt'):
        raise Exception("ERROR: terabox.txt not found")
    ...
    result = result[0]
    if result['isdir'] != '0':
        raise Exception("ERROR: Can't download folder")
    return result['dlink']
...
def filepress(url):
    ...
    try:
        ...
        raise Exception(f'ERROR: {e.__class__.__name__}')
    if 'data' not in res:
        raise Exception(f'ERROR: {res["statusText"]}')
...
def gdtot(url):
    ...
    if not path:
        raise Exception('ERROR: Cannot bypass this')
    path = path[0]
...
def wetransfer(url):
    ...
    if "direct_link" in res:
        ...
        raise Exception("ERROR: cannot find direct link")
...
def linkbox(url):
    ...
    if 'itemInfo' not in data:
        raise Exception('ERROR: itemInfo not found!!')
    itemInfo = data['itemInfo']
    if 'url' not in itemInfo:
        raise Exception('ERROR: url not found in itemInfo!!')
